...
def training_stage(model, train_loader, optimizer, epochs=128):
    ...
    for i in tqdm(range(epochs)):
        for k, (data, label) in enumerate(train_loader):
            ...
            hidden_layer_mean = torch.mean(model.encoder(data), dim=0)
            sparsity_loss = Sparsity_Weight * torch.sum(kl_divergence(Sparsity_Target, hidden_layer_mean))
            batch_loss = F.l1_loss(cell_prop, label) + F.l1_loss(x_recon, data) + sparsity_loss
            batch_loss.backward()
            optimizer.step()
